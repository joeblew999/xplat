...
func TestGlobRelativePath(t *testing.T) {
	...
	// Create test files
	_ = os.MkdirAll("src/pkg", 0755)
	_ = os.WriteFile("src/main.go", []byte("package main"), 0644)
	_ = os.WriteFile("src/pkg/util.go", []byte("package pkg"), 0644)
...
func TestGlobAbsolutePath(t *testing.T) {
	...
	// Create test files
	_ = os.MkdirAll(filepath.Join(tmpDir, "nested", "dir"), 0755)
	_ = os.WriteFile(filepath.Join(tmpDir, "file1.txt"), []byte("1"), 0644)
	_ = os.WriteFile(filepath.Join(tmpDir, "nested", "file2.txt"), []byte("2"), 0644)
	_ = os.WriteFile(filepath.Join(tmpDir, "nested", "dir", "file3.txt"), []byte("3"), 0644)
...
func TestGlobSpecialPatterns(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(tmpDir, "file.md"), []byte("2"), 0644)
	_ = os.WriteFile(filepath.Join(tmpDir, "other.txt"), []byte("3"), 0644)

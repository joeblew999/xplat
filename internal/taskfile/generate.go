...
func GenerateGitignore(outputPath string, binaryName string) error {
	const gitignoreTmpl = `# Generated by: xplat manifest bootstrap

# Binary (root level only, so cmd/binary/ is not ignored)
/{{.Binary}}
*.exe
...
`
	...
	tmpl, err := template.New("gitignore").Parse(gitignoreTmpl)
	if err != nil {
		return fmt.Errorf("failed to parse template: %w", err)
	}

	var buf bytes.Buffer
	if err := tmpl.Execute(&buf, map[string]string{"Binary": binaryName}); err != nil {
